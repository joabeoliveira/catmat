...
export async function GET(request: Request) {
  ...
  try {
    const rows = await prisma.$queryRawUnsafe<Array<{ sugestao: string }>>(
      `
        SELECT "nomeServico" AS sugestao
        FROM "CatserItem"
        WHERE "nomeServico" ILIKE $1 ESCAPE '\\'
           OR similarity(immutable_unaccent("nomeServico"), immutable_unaccent($2)) > 0.12
        GROUP BY "nomeServico"
        ORDER BY
          CASE WHEN "nomeServico" ILIKE $1 ESCAPE '\\' THEN 0 ELSE 1 END,
          similarity(immutable_unaccent("nomeServico"), immutable_unaccent($2)) DESC,
          count(*) DESC
        LIMIT 8
      `,
