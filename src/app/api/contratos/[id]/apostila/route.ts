...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const methodology = 'a) Lei nº 14.133, de 1º de abril de 2021: art. 136. Os registros que não caracterizam alteração do contrato podem ser realizados por simples apostila, dispensada a celebração de termo aditivo, especialmente a variação do valor contratual para fazer face ao reajuste ou à repactuação de preços previstos no próprio contrato. b) Decreto nº 1.054, de 7 de fevereiro de 1994: art. 1º. O reajuste de preços nos contratos firmados pelos órgãos e entidades da Administração Pública rege-se pelo disposto neste decreto. Conforme o art. 5º, os preços contratuais serão reajustados para mais ou para menos de acordo com a variação do índice indicado no instrumento convocatório ou no contrato, vedada periodicidade inferior a um ano, contada da data limite para apresentação da proposta. O cálculo observa a fórmula R = V x (I - I₀) / I₀, em que R é o valor do reajuste procurado; V é o valor contratual do fornecimento, obra ou serviço a ser reajustado; I₀ é o índice inicial, correspondente à data fixada para entrega da proposta da licitação; e I é o índice de referência na data do reajuste. Os índices utilizados e os valores resultantes estão demonstrados na memória de cálculo e devem ser conferidos no processo administrativo.'
    ...
    const doc = new Document({
      ...
      sections: [{
        ...
        children: [
          ...
          ...clause(2, 'DO FUNDAMENTO E DA METODOLOGIA', methodology),
          ...
          ...clause(firstLaterClause + 1, 'DA GARANTIA CONTRATUAL', `Caso o contrato preveja garantia contratual, o(a) CONTRATADO(A) deverá providenciar sua complementação ou renovação para manter o percentual exigido sobre o novo valor contratual reajustado. O valor mínimo da garantia deverá ser conferido pela aplicação do percentual previsto no contrato sobre o novo valor de ${money(calculation.newValue)}. O comprovante deverá ser apresentado ao órgão contratante no prazo estabelecido no instrumento original ou em sua convocação.`),
          ...clause(firstLaterClause + 2, 'DA RATIFICAÇÃO', 'Ficam ratificadas as demais cláusulas e condições do contrato original e de seus aditivos que não tenham sido alteradas por este Termo de Apostilamento.'),
          ...clause(firstLaterClause + 3, 'DA PUBLICAÇÃO E DA EFICÁCIA', 'O presente Termo de Apostilamento produzirá efeitos a partir da data definida no processo administrativo, sem prejuízo da observância das providências de publicidade e divulgação exigidas pela legislação aplicável.'),
          ...
          new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 550 }, children: [new TextRun({ text: 'Local e data: ________________________________________________', font: 'Aptos', size: 21 })] }),
          new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 700 }, children: [new TextRun({ text: '____________________________________________________________', font: 'Aptos', size: 21 })] }),
          ...
          new Paragraph({ alignment: AlignmentType.CENTER, spacing: { before: 450 }, children: [new TextRun({ text: '____________________________________________________________', font: 'Aptos', size: 21 })] }),
