    ...
    <main className="min-h-screen bg-slate-950 px-4 py-10 text-slate-100">
      <div className="mx-auto flex max-w-5xl flex-col gap-6">
        <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6">
          <p className="text-sm uppercase tracking-[0.3em] text-cyan-400">Sua lista</p>
        ...
        {carregado && favoritos.length === 0 && (
          <Card>
            <CardContent className="pt-6 text-center text-slate-400">
              Nenhum favorito ainda. <Link href="/" className="text-cyan-400 hover:underline">Busque um material</Link>{' '}
          ...
          <Card key={item.codigoItem}>
            <CardContent className="flex flex-col gap-3 pt-6 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0">
                ...
                <div className="mt-1 font-medium text-white">{item.descricaoItem}</div>
              </div>
              <div className="flex shrink-0 items-center gap-2">
                <Link
                  href={`/material/${item.codigoItem}`}
                  className="inline-flex items-center rounded-md border border-slate-700 px-3 py-2 text-sm text-slate-300 transition hover:border-cyan-500 hover:text-cyan-300"
                >
                  Ver detalhes →
                </Link>
