    ...
    <main className="min-h-screen bg-slate-950 px-4 py-10 text-slate-100">
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
        <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6">
          <p className="text-sm uppercase tracking-[0.3em] text-cyan-400">Navegação por categoria</p>
          <h1 className="mt-3 text-3xl font-semibold text-white">Grupos de materiais</h1>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {grupos.map((grupo) => (
            <Link
              ...
              className="rounded-2xl border border-slate-800 bg-slate-900/80 p-5 transition hover:border-cyan-500"
            >
              <div className="text-xs uppercase tracking-wide text-slate-500">Grupo {grupo.codigo}</div>
              <div className="mt-2 font-medium leading-snug text-white">{grupo.nome}</div>
              <div className="mt-3 text-sm text-slate-400">{grupo.quantidade.toLocaleString('pt-BR')} itens</div>
            </Link>
          ))}
        </div>
      </div>
