    ...
    <main className="min-h-screen bg-slate-950 px-4 py-10 text-slate-100">
      <div className="mx-auto flex max-w-5xl flex-col gap-6">
        <nav aria-label="Caminho" className="flex flex-wrap items-center gap-2 text-sm text-slate-400">
          ...
          <span>/</span>
          <Link href={`/?grupo=${item.codigoGrupo}`} className="hover:text-cyan-400">{item.nomeGrupo}</Link>
          <span>/</span>
          <Link href={`/?classe=${item.codigoClasse}`} className="hover:text-cyan-400">{item.nomeClasse}</Link>
          <span>/</span>
          <span className="text-slate-300">CATMAT {item.codigoItem}</span>
        </nav>
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6">
          ...
          <div className="mt-4 flex flex-wrap gap-3 text-sm text-slate-400">
            ...
            {item.codigoNcm && <span className="rounded-full border border-slate-700 px-3 py-1">NCM {item.codigoNcm}</span>}
          </div>
        </div>

        {caracteristicas.length > 0 && (
          <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6">
            <h2 className="text-xl font-semibold text-white">Características</h2>
            <dl className="mt-4 grid gap-x-6 gap-y-3 text-sm sm:grid-cols-2">
              {caracteristicas.map((caracteristica) => (
                <div key={caracteristica.chave} className="rounded-lg border border-slate-800 bg-slate-950/50 p-3">
                  <dt className="text-xs uppercase tracking-wide text-slate-500">{caracteristica.chave}</dt>
                  <dd className="mt-1 font-medium text-white">{caracteristica.valor}</dd>
                </div>
              ))}
            </dl>
          </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6">
            <h2 className="text-xl font-semibold text-white">Sobre este material</h2>
            <p className="mt-3 text-sm leading-6 text-slate-300">
              O material de código <strong className="text-white">CATMAT {item.codigoItem}</strong> pertence ao
              grupo <strong className="text-white">{item.nomeGrupo}</strong>, classe{' '}
              <strong className="text-white">{item.nomeClasse}</strong>, sob o padrão descritivo de material (PDM){' '}
              <strong className="text-white">{item.nomePdm}</strong>
              {item.codigoNcm ? <> e NCM <strong className="text-white">{item.codigoNcm}</strong></> : null}.
        ...
        {comprasRecentes.length > 0 && (
          <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-6">
            <h2 className="text-xl font-semibold text-white">Histórico de licitações</h2>
            <p className="mt-1 text-sm text-slate-400">Compras públicas mais recentes registradas para este item.</p>
            <div className="mt-4 grid gap-3">
              {comprasRecentes.map((compra, index) => (
                <div key={index} className="flex flex-col gap-2 rounded-xl border border-slate-800 bg-slate-950/50 p-4 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0 text-sm">
                    {compra.orgao && <div className="font-medium text-white">{compra.orgao}{compra.uasg && compra.uasg !== compra.orgao ? ` — ${compra.uasg}` : ''}</div>}
                    {compra.fornecedor && (
                  ...
                  <div className="shrink-0 text-right">
                    <div className="text-lg font-semibold text-white">{formatarMoeda(compra.precoUnitario)}</div>
                    {compra.unidade && <div className="text-xs text-slate-500">/{compra.unidade}</div>}
                    <div className="mt-1 text-xs text-slate-400">{formatarData(compra.dataCompra)}</div>
                  </div>
                </div>
              ))}
            </div>
