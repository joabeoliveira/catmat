...
import { CnpjLink } from '@/components/cnpj/CnpjLink'
import { ModuleHeader } from '@/components/shared/ModuleHeader'
import { ArpDescricaoInput } from './ArpDescricaoInput'
...
export function ArpSearch() {
  ...
  const [adesaoItems, setAdesaoItems] = useState<Item[]>([]); const [adesaoTotal, setAdesaoTotal] = useState(0); const [adesaoLoading, setAdesaoLoading] = useState(true); const [adesaoSyncing, setAdesaoSyncing] = useState(false); const [adesaoSyncInfo, setAdesaoSyncInfo] = useState('')
  ...
  async function loadHome(page = 1, query = '') {
    setHomeLoading(true)
    try {
      const data = await localApi({ pagina: String(page), limite: '200', q: query.trim() })
  ...
  async function loadAdesao(page = 1, values: SearchValues = { uasg: '', catmat: '', uf: '', objeto: '' }) {
    ...
    try {
      ...
      setAdesaoItems(data.resultado || []); setAdesaoTotal(data.totalRegistros || 0); setPagina(page); setPaginas(data.totalPaginas || 1)
  ...
  useEffect(() => { try { setHistory(JSON.parse(localStorage.getItem('catmat:arp:historico') || '[]')) } catch {} }, [])
  useEffect(() => {
    if (searched) return
    const timer = window.setTimeout(() => {
      if (aba === 'adesao') void loadAdesao(1, { uasg: '', catmat: '', uf: '', objeto: homeQuery })
      else void loadHome(1, homeQuery)
    }, homeQuery.trim() ? 300 : 0)
    return () => window.clearTimeout(timer)
  }, [aba, homeQuery, searched])

  async function search(event?: React.FormEvent, values: SearchValues = { uasg, catmat, uf, objeto }, page = 1, refinamentoAtual = refinamento) {
    ...
    try {
      const termoBusca = [values.objeto, refinamentoAtual].map((valor) => valor.trim()).filter(Boolean).join(' ')
      const data = aba === 'adesao' ? await (async () => { const res = await fetch(`/api/arp/adesao?${new URLSearchParams({ pagina: String(page), limite: '200', q: termoBusca, uasg: values.uasg, catmat: values.catmat })}`); const json = await res.json(); if (!res.ok) throw new Error(json.error || 'Falha ao buscar oportunidades.'); return json })() : await localApi({ pagina: String(page), limite: '200', q: termoBusca, uasg: values.uasg, catmat: values.catmat, uf: values.uf }); let result = (data.resultado || []).map((i: Item) => ({ ...(i.ata || {}), ...i, valorTotalAta: i.ata?.valorTotal }))
  ...
  const display = aba === 'adesao' ? (searched ? items : adesaoItems) : (searched ? items : homeItems)
  ...
  return <main ref={buscaRef} className="min-h-screen bg-slate-50 px-3 py-6 text-slate-900 dark:bg-slate-950 dark:text-slate-100 sm:px-4 sm:py-10"><div className="mx-auto max-w-7xl space-y-6">
    ...
    {!searched && aba === 'adesao' && <section className="rounded-2xl border border-indigo-100 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="flex items-center gap-2 text-lg font-semibold"><PackageSearch className="h-5 w-5 text-indigo-600" />Itens disponíveis para adesão</h2><p className="mt-1 text-sm text-slate-500">Itens com máximo de adesão maior que zero, armazenados localmente.</p></div><div className="flex gap-2"><button onClick={syncAdesao} disabled={adesaoSyncing} className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-3 py-2 text-sm text-white hover:bg-indigo-700 disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${adesaoSyncing ? 'animate-spin' : ''}`} /> {adesaoSyncing ? `Carregando lote ${adesaoSyncPage}…` : `Carregar lote ${adesaoSyncPage}`}</button><button onClick={() => void loadAdesao()} disabled={adesaoLoading} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm hover:bg-slate-50 dark:border-slate-700"><RefreshCw className={`h-4 w-4 ${adesaoLoading ? 'animate-spin' : ''}`} /> Atualizar</button></div></div><div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-900 dark:bg-emerald-950/30"><p className="text-xs font-medium uppercase tracking-wide text-emerald-700 dark:text-emerald-300">Itens disponíveis para adesão</p><p className="mt-1 text-3xl font-bold text-emerald-800 dark:text-emerald-200">{adesaoTotal.toLocaleString('pt-BR')}</p><p className="text-xs text-emerald-700 dark:text-emerald-300">Registros ativos na base local</p></div>{adesaoSyncInfo && <p role="status" className="mt-3 text-sm">{adesaoSyncInfo} Use “Carregar lote” para trazer a próxima página da API.</p>}<div className="mt-4"><ArpDescricaoInput value={homeQuery} onChange={setHomeQuery} placeholder="Filtre pela descrição do item…" /></div></section>}
    {!searched && aba === 'atas' && <section className="rounded-2xl border border-indigo-100 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="flex items-center gap-2 text-lg font-semibold"><PackageSearch className="h-5 w-5 text-indigo-600" />Atas vigentes</h2><p className="mt-1 text-sm text-slate-500">Atas armazenadas. Abra os detalhes para consultar e atualizar seus itens.</p></div><div className="flex gap-2"><button onClick={syncDatabase} disabled={syncing} className="inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-3 py-2 text-sm text-white hover:bg-indigo-700 disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${syncing ? 'animate-spin' : ''}`} /> {syncing ? 'Sincronizando…' : 'Sincronizar API'}</button><button onClick={() => void loadHome()} disabled={homeLoading} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm hover:bg-slate-50 dark:border-slate-700"><RefreshCw className={`h-4 w-4 ${homeLoading ? 'animate-spin' : ''}`} /> Atualizar</button></div></div><div className="mt-4"><ArpDescricaoInput value={homeQuery} onChange={setHomeQuery} placeholder="Filtre pela descrição do item…" /></div></section>}
    {searched && <div className="flex flex-col gap-3 rounded-xl border border-indigo-100 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:flex-row sm:items-end sm:justify-between"><div><h2 className="text-xl font-semibold">Resultados encontrados ({items.length}){aba === 'adesao' && <span className="ml-2 text-sm font-normal text-emerald-700 dark:text-emerald-400">· {adesaoTotal.toLocaleString('pt-BR')} disponíveis na base</span>}</h2><p className="mt-1 text-sm text-slate-500">Refine a descrição sem perder a busca original.</p></div><form className="flex w-full flex-col gap-2 sm:max-w-md sm:flex-row" onSubmit={(event) => { event.preventDefault(); void search(undefined, applied, 1, refinamento) }}><input value={refinamento} onChange={(event) => setRefinamento(event.target.value)} placeholder="Ex.: 20 ml, descartável, branco" aria-label="Refinar resultados da busca" className="min-h-11 min-w-0 flex-1 rounded-lg border border-slate-300 bg-white px-3 text-sm dark:border-slate-700 dark:bg-slate-950" /><button type="submit" disabled={loading} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50"><Search className="h-4 w-4" />Refinar</button></form></div>}{(homeLoading || adesaoLoading) && !searched && <p className="text-sm text-slate-500">Consultando {aba === 'adesao' ? 'oportunidades de adesão' : 'atas vigentes'}…</p>}{!loading && !homeLoading && !adesaoLoading && display.length === 0 && <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center text-sm text-slate-500 dark:border-slate-800 dark:bg-slate-900">Nenhum resultado encontrado.</div>}
    <div className="grid gap-4 pb-20 sm:pb-0 lg:grid-cols-2">{display.map((item, i) => <article key={`${item.numeroControlePncpAta}-${item.numeroItem}-${i}`} className="min-w-0 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900"><div className="flex min-w-0 items-start gap-3"><div className="rounded-lg bg-indigo-50 p-2 text-indigo-600 dark:bg-indigo-950"><PackageSearch className="h-5 w-5" /></div><div className="min-w-0 flex-1"><h3 className="min-w-0 break-words font-semibold">{item.descricaoItem || item.objeto || 'Item sem descrição'}</h3><p className="mt-1 break-words text-xs text-slate-500">Ata {item.numeroAtaRegistroPreco} · {item.numeroItem && item.numeroItem !== '—' ? `Item ${item.numeroItem} · CATMAT ${item.codigoItem || '—'}` : 'Resumo da ata'}</p></div><span className="shrink-0 whitespace-nowrap text-sm font-semibold">{money(item.valorUnitario ?? item.valorTotal)}</span></div><div className="mt-4 grid grid-cols-2 gap-2 text-xs text-slate-600 dark:text-slate-400"><span className="min-w-0 break-words">Órgão: <b>{item.codigoUnidadeGerenciadora}</b></span><span className="min-w-0 break-words">{item.numeroItem === '—' ? 'Itens na ata' : 'Quantidade homologada'}: <b>{item.quantidadeHomologadaItem ?? item.quantidadeHomologadaVencedor ?? item.quantidadeItens ?? '—'}</b></span>{aba === 'adesao' && <span className="min-w-0 break-words font-semibold text-emerald-700 dark:text-emerald-400">Máximo para adesão: <b>{item.maximoAdesao}</b></span>}<span className="flex min-w-0 items-center gap-1 break-words"><CalendarDays className="h-3.5 w-3.5 shrink-0" /> Até {dateBR(item.dataVigenciaFinal)}</span><span className="flex min-w-0 items-center gap-1 break-words"><MapPin className="h-3.5 w-3.5 shrink-0" /> {item.siglaUf || 'UF não informada'}</span></div><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => setSelected(item)} className="rounded-lg bg-indigo-600 px-3 py-2 text-xs font-medium text-white hover:bg-indigo-700">Ver detalhes</button>{pncp(item) && <a href={pncp(item)!} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-3 py-2 text-xs dark:border-slate-700"><ExternalLink className="h-3.5 w-3.5" /> PNCP</a>}</div></article>)}</div>
    <div className="flex items-center justify-between gap-3"><button disabled={loading || homeLoading || adesaoLoading || pagina <= 1} onClick={() => searched ? void search(undefined, applied, pagina - 1, refinamento) : aba === 'adesao' ? void loadAdesao(pagina - 1) : void loadHome(pagina - 1)}>Anterior</button><span>Página {pagina} de {paginas}</span><button disabled={loading || homeLoading || adesaoLoading || pagina >= paginas} onClick={() => searched ? void search(undefined, applied, pagina + 1, refinamento) : aba === 'adesao' ? void loadAdesao(pagina + 1) : void loadHome(pagina + 1)}>Próxima</button></div>
    ...
    {display.length > 0 && <button type="button" onClick={() => buscaRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className="fixed bottom-4 right-3 z-20 inline-flex min-h-11 max-w-[calc(100vw-1.5rem)] items-center gap-2 rounded-full bg-indigo-600 px-3 py-2 text-sm font-medium text-white shadow-lg transition hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:bottom-5 sm:right-5 sm:px-4" aria-label="Voltar para a pesquisa"><ArrowUp className="h-4 w-4 shrink-0" /><span className="hidden sm:inline">Voltar para pesquisa</span></button>}
  ...
  return <section className="mt-6 rounded-xl border border-indigo-300 p-5">
    ...
    <dl className="mt-4 grid gap-4 sm:grid-cols-2">
      {Object.entries({
        ...
        'Fornecedor': item.nomeRazaoSocialFornecedor, 'CNPJ/CPF': <CnpjLink cnpj={item.niFornecedor} />,
