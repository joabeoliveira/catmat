...
export function FavoritosLink() {
  ...
  return (
    <Link
      href="/favoritos"
      className="inline-flex items-center gap-2 rounded-md border border-slate-800 px-3 py-1.5 text-sm text-slate-300 transition hover:border-rose-500/50 hover:text-rose-400"
      aria-label={`Favoritos (${quantidade})`}
    >
      <Heart className="h-4 w-4" />
      <span className="hidden sm:inline">Favoritos</span>
      {quantidade > 0 && (
        <span className="rounded-full bg-rose-500/20 px-1.5 text-xs font-medium text-rose-300">{quantidade}</span>
      )}
    </Link>
  )
}
